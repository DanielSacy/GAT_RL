...
def update_mask(demand,capacity,selected,mask,i):
    ...
    # This condition checks if the vehicle cannot serve any of the nodes not blocked by the mask
    # cannot_serve_any = (demand * (1 - mask)).max(dim=1)[0] > capacity.squeeze(-1)
    # if cannot_serve_any.any():
    #     mask[torch.logical_not(go_depot), 0] = 0
